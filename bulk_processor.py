...
class BulkResumeProcessor:
    ...
    def process_bulk_resumes(
        ...
        """
        Processes a batch of resume PDFs (e.g. 50-100 files).
        Returns a ranked summary of all evaluated candidates.
        """
        processed_candidates = []
        errors = []

        for filename, content_bytes in file_tuples:
            try:
                pdf_stream = io.BytesIO(content_bytes)
                parsed = parse_resume_pdf(pdf_stream)

                if not parsed.get("success"):
                    errors.append({"filename": filename, "error": parsed.get("error")})
                    continue
...
if __name__ == "__main__":
    print("BulkResumeProcessor module ready.")
